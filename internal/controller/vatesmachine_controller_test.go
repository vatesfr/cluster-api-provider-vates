...
var _ = Describe("VatesMachine Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind VatesMachine")
			err := k8sClient.Get(ctx, typeNamespacedName, vatesMachine)
			if err != nil && errors.IsNotFound(err) {
				resource := &infrastructurev1beta2.VatesMachine{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance VatesMachine")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &VatesMachineReconciler{
				Client: k8sClient,
